...
public class Utils { 
	...
	public static boolean localmonotonicity_check(GraphNode gnode, HashMap<Identifier,GraphNode> visitedList){
		float hc;
		float c;
		float hp;
		for(int i=0; i<gnode.neighborNodes.size(); i++){
			hc = visitedList.get(gnode.neighborNodes.get(i)).hVal;
